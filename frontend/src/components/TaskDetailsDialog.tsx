...
const TaskDetailsDialog = ({ 
  ...
  // console.log("TaskDetailsDialog rendered with task:", task, "open status:", open);
  ...
  useEffect(() => {
    if (task) {
      // console.log("Setting form data from task:", task);
      setFormData({
        title: task.title,
      ...
      // Reset any file preview when a new task is loaded
      setFilePreview(null);
      
      // If the task has a fileUrl and it's an image, try to set the preview
      if (task.fileUrl && task.fileName && isImageFile(task.fileName)) {
        setFilePreview(task.fileUrl);
      }
    }
  }, [task]);
        ...
        {!task ? (
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                Task information could not be loaded. Please try again.
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="space-y-6 mt-4">
                ...
                {task.fileUrl ? (
                  <div className="border rounded-md overflow-hidden">
                    ...
                    {/* Only try to display preview for image files */}
                    {task.fileName && isImageFile(task.fileName) ? (
